...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = s->pc + imm);  // for dummy
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + imm); 
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4; s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4; s->dnpc = (src1 + imm) &~ 1);

  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1 == src2) s->dnpc = s->pc + imm); // for add
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = src1 + src2); 
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = src1 - src2); 
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (src1 < imm));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2) s->dnpc = s->pc + imm);

  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((sword_t)src1 >= (sword_t)src2) s->dnpc = s->pc + imm); // for bubble-sort
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, if(((imm >> 5) & 1) == 0) R(dest) = src1 << imm); // ? 
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = (src1 | src2)); 
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = (src1 << (src2 & 0x1f))); 
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = src1 ^ imm); 
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, if(((imm >> 5) & 1) == 0) R(dest) = (src1 >> imm)); // ? 
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, if(((imm >> 5) & 1) == 0) R(dest) = (sword_t)src1 >> imm); // ?
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(dest) = ((sword_t)src1 < imm)); 
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, if(imm==0x300){R(dest)=csr_msta; csr_msta=src1;}
                                                           ...
                                                           else if(imm==0x340){R(dest)=csr_mscratch; csr_mscratch=src1;}
                                                                                                                ); 
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, if(imm==0x300){R(dest)=csr_msta; csr_msta=csr_msta|src1;}
                                                           ...
                                                           else if(imm==0x340){R(dest)=csr_mscratch; csr_mscratch=csr_mscratch|src1;}
                                                                                                                          ); 
